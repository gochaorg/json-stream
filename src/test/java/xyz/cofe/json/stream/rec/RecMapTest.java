...
public class RecMapTest {
    ...
    public void primitives(){
        ...
        var bool_str = mapper.toJsonString(bool_src);
        System.out.println(bool_str);
        Boolean bool_dst = mapper.fromJson(bool_str, Boolean.class);
        assertTrue(bool_dst);

        byte byte_src = (byte) 140;
        var byte_str = mapper.toJsonString(byte_src);
        System.out.println(byte_str);
        Byte byte_dst = mapper.fromJson(byte_str, Byte.class);
        assertTrue(Objects.equals(byte_dst,byte_src));

        short short_src = (short) 140;
        var short_str = mapper.toJsonString(short_src);
        System.out.println(short_str);
        Short short_dst = mapper.fromJson(short_str, Short.class);
        assertTrue(Objects.equals(short_dst,short_src));

        int int_src = (int) 140;
        var int_str = mapper.toJsonString(int_src);
        System.out.println(int_str);
        Integer int_dst = mapper.fromJson(int_str, Integer.class);
        assertTrue(Objects.equals(int_dst,int_src));

        long long_src = (long) 14056;
        var long_str = mapper.toJsonString(long_src);
        System.out.println(long_str);
        Long long_dst = mapper.fromJson(long_str, Long.class);
        assertTrue(Objects.equals(long_dst,long_src));

        float float_src = (float) 12.34;
        var float_str = mapper.toJsonString(float_src);
        System.out.println(float_str);
        Float float_dst = mapper.fromJson(float_str, Float.class);
        assertTrue(Objects.equals(float_dst,float_src));

        double double_src = (double) 123.34;
        var double_str = mapper.toJsonString(double_src);
        System.out.println(double_str);
        Double double_dst = mapper.fromJson(double_str, Double.class);
        assertTrue(Objects.equals(double_dst,double_src));

        char char_src = (char) 'a';
        var char_str = mapper.toJsonString(char_src);
        System.out.println(char_str);
        Character char_dst = mapper.fromJson(char_str, Character.class);
